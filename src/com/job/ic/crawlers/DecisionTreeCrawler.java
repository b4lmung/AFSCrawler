...
public class DecisionTreeCrawler {
	private static Logger logger = Logger.getLogger(DecisionTreeCrawler.class);
	...
	public static synchronized void addPage(final double score, final String url) {
		...
		if (DecisionTreeCrawler.rel + DecisionTreeCrawler.non > DecisionTreeCrawler.limitPage) {
			System.exit(0);
		}
	...
	public static void main(final String[] args) throws Exception {
		...
		System.out.println(String.valueOf(DecisionTreeCrawler.rel) + "\t" + DecisionTreeCrawler.non);
	...
	public static void softFocusedThread() {
		...
		while (queue.size() > 0) {
			...
			DecisionTreeCrawler.logger.info(String.format("%s\t%.2f\t%s\tParent:\t%.2f", url, score, progress(), qObj.getScore()));
			...
			for (final LinksModel l : links) {
				...
				if (DecisionTreeCrawler.isBFS) {
					queue.add(new QueueProxyObj(l.getLinkUrl(), url, 0, qObj.getDepth() + 1, 0));
				} else {
					try {
						String[] features = { "a", "b", l.getAnchorText(), "non" };
...
//						else
//							queue.add(new QueueProxyObj(l.getLinkUrl(), url, pscore, qObj.getDepth() + 1, qObj.getDistanceFromRelevantPage() + 1));
					} catch (Exception e) {
		...
		DecisionTreeCrawler.logger.info("finished");
	...
	public static WekaClassifier trainClassifier(String trainPath) {
		...
		WekaClassifier c = (WekaClassifier) FileUtils.getObjFile("d3-tourism.model");
		return c;

		// WekaClassifier c = new WekaClassifier("weka.classifiers.trees.J48",
		// "-C 0.4 -M 2");
		...
		// }
		//
		// StringToWordVector vector = new StringToWordVector(2000);
		// vector.setOutputWordCounts(false);
		...
		//
		// Filter[] filters = new Filter[]{rm, order, vector};
		// c.train("d3-tourism-uns.arff", null, true, filters, true, false);
		// FileUtils.saveObjFile(c, "d3-tourism.model");

		// return anchor;
	}

	public static void analyzingTrainingData() throws MalformedURLException {

		DirectedGraph<String, DefaultEdge> graph = new DefaultDirectedGraph<>(DefaultEdge.class);
		ProxyService.setupProxy("proxy-train-tourism");

		// add seeds
		ArrayList<String> seeds = new ArrayList<>();

		for (WebsiteSegment s : FileUtils.readSegmentFile("back-tourism-s.txt")) {
			seeds.addAll(s.getUrls());
		}

		ArrayList<String> allUrls = ProxyService.getAll();

		// find only relevant page

		HashSet<String> relevant = new HashSet<>();

		// add nodes
		for (String s : allUrls) {
			ProxyModel node = ProxyService.retreiveContentByURL(s, null);
			if (node == null)
				continue;

			graph.addVertex(node.getUrl().toLowerCase());
			if (node.getScore() > 0.5) {
				// relevant
				relevant.add(node.getUrl());
			}
		}
		...
		for (String s : allUrls) {
			...
			for (LinksModel l : node.getLinks()) {
				if (ProxyService.contains(l.getSourceUrl().toLowerCase()) && ProxyService.contains(l.getLinkUrl().toLowerCase()) && l.getAnchorText() != null && l.getAnchorText().trim().length() > 0) {
					try {
						graph.addEdge(l.getSourceUrl().toLowerCase(), l.getLinkUrl().toLowerCase());
					} catch (Exception e) {
		...
		HashSet<DefaultEdge> relEdge = new HashSet<>();
		// for each seed
		...
		for (String s : seeds) {
			...
			logger.info(i++ + "/" + seeds.size() + "\t" + relEdge.size());
			// for each relevant page
			for (String k : relevant) {
				k = k.toLowerCase();
				// logger.info("finding path for " + s + "->" + k + "\t" +
				// graph.containsVertex(s) + "\tt" + graph.containsVertex(k));

				if (graph.containsVertex(s) && graph.containsVertex(k)) {
					DijkstraShortestPath<String, DefaultEdge> path = new DijkstraShortestPath<String, DefaultEdge>(graph, s, k);
					if (path == null || path.getPathEdgeList() == null)
						continue;

					relEdge.addAll(path.getPathEdgeList());
				}
			}
		}
		...
		// System.out.println(relEdge.size());
		// System.out.println(nonEdge.size());
		// relevant features

		for (DefaultEdge s : relEdge) {
			String srcUrl = graph.getEdgeSource(s);
			String destUrl = graph.getEdgeTarget(s);
			ProxyModel m = ProxyService.retreiveContentByURL(srcUrl, null);
			if (m == null || m.getLinks() == null)
				continue;

			for (LinksModel l : m.getLinks()) {
				if (l.getAnchorText() == null || l.getAnchorText().trim().equals(""))
					continue;

				if (l.getLinkUrl().toLowerCase().equals(destUrl)) {
					positive.add(String.format("'%s','%s','%s',thai", srcUrl, destUrl, StringUtils.cleanAnchorText(l.getAnchorText())));
				}
			}
		}

		for (DefaultEdge s : graph.edgeSet()) {
			if (relEdge.contains(s))
				continue;
			...
			for (LinksModel l : m.getLinks()) {
				...
				if (l.getLinkUrl().toLowerCase().equals(destUrl)) {
					negative.add(String.format("'%s','%s','%s',non", StringUtils.cleanUrlDataForPrediction(srcUrl), StringUtils.cleanUrlDataForPrediction(destUrl),
							StringUtils.cleanAnchorText(l.getAnchorText())));
				}
			}
		}

		FileUtils.writeTextFile("d-tourism.arff", header, false);
		FileUtils.writeTextFile("d-tourism.arff", positive, true);
		FileUtils.writeTextFile("d-tourism.arff", negative, true);

		ArrayList<String> tmp = FeaturesCollectors.underSampling(new ArrayList<>(positive),new ArrayList<>(negative));

		FileUtils.writeTextFile("d-tourism-uns.arff", header, false);
		FileUtils.writeTextFile("d-tourism-uns.arff", tmp, true);
		ProxyService.terminateProxy();
	}

	public static void trainD3Classifier() {
		// WekaClassifier w = new WekaClassifier("weka.classifiers.trees.J48",
		// "-C 0.4 -M 2");
		...
		// int[] rm = new int[]{1,2};
		// w.train("test.arff", rm, false, f, true, true);
