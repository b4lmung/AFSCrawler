...
//		String siteurl = HttpUtils.getHost(seed[0]);
		...
		SocketHTTPFetcher hp = new SocketHTTPFetcher(SiteCrawler.getClient());
...
//					if (CrawlerConfig.getConfig().getSegmentThreshold() >= 0 && nonThreshold >= CrawlerConfig.getConfig().getSegmentThreshold()) {
//						break;
//					}
					
					if (CrawlerConfig.getConfig().getMaxDepth() >= 0 && obj.getDepth() > CrawlerConfig.getConfig().getMaxDepth()) {
						continue;
					}
					
					if (CrawlerConfig.getConfig().getMaxPagePerSite() >= 0 && (thai+non) >= CrawlerConfig.getConfig().getMaxPagePerSite()) {
						break;
					}
...
//						System.out.println(siteurl + "\t" + l.getLinkUrl() + "\t" + l.getLinkUrl().replace("http://", "").replace("https://","").startsWith(siteurl));
//						System.out.println(l.getLinkUrl().startsWith(siteurl));
						if(l.getLinkUrl().startsWith(siteurl))
						{
							nodeq.enQueue(new QueueObj(l.getLinkUrl(), fp.getUrl(), fp.getDepth()+1, 0, fp.getPageScore()));
						}
					}
					...
					logger.info("DOWNLOADED\t" + fp.getPageScore() + "\t" + obj.toString() + "\t" + (thai+non)+ "\tfrom:\t" + base);
