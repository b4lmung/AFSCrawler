/* Robots.java
 *
 * $Id: Robotstxt.java 6824 2010-04-13 22:43:44Z gojomo $
 *
 * Created Sep 1, 2005
 *
...
public class RobotstxtParser implements Serializable {
	...
	public RobotstxtParser(String[] lines) throws IOException {
		...
		for (String read : lines) {
			...
			if ((read = read.trim()).startsWith("#") || read.length() == 0)
				continue;
	...
	public RobotsDirectives getDirectives() {
		// find matching ua
		for (String uaListed : userAgents) {
			if (CrawlerConfig.getConfig().getUsrAgent().indexOf(uaListed) > -1) {
