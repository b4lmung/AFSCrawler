...
//		FileUtils.deleteDir("urlDb");
		...
		// seeds
		LinkedList<String[]> queue = com.job.ic.utils.FileUtils.importQueueFromFile(CrawlerConfig.getConfig().getSeedPath(), null);
		
		// number of threads = 8
		final Core crawler = new Core(queue, CrawlerConfig.getConfig().getNumThreads());

		crawler.start();

	}

	public Core(LinkedList<String[]> queue, int numThreads) {
