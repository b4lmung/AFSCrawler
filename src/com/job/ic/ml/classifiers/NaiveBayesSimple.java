 ...
 */
public class NaiveBayesSimple extends AbstractClassifier implements TechnicalInformationHandler {
	...
	public void buildClassifier(Instances instances) throws Exception {
		...
		while (enumInsts.hasMoreElements()) {
			...
			if (!instance.classIsMissing()) {
				...
				while (enumAtts.hasMoreElements()) {
					...
					if (!instance.isMissing(attribute)) {
						if (attribute.isNumeric()) {
							m_Devs[(int) instance.classValue()][attIndex] += (m_Means[(int) instance.classValue()][attIndex] - instance.value(attribute))
									* (m_Means[(int) instance.classValue()][attIndex] - instance.value(attribute));
						}
					}
					attIndex++;
			...
			if (attribute.isNominal()) {
				for (int j = 0; j < instances.numClasses(); j++) {
					...
					for (int i = 0; i < attribute.numValues(); i++) {
						m_Counts[j][attIndex][i] = (m_Counts[j][attIndex][i] + 1) / (sum + (double) attribute.numValues());
					}
				}
			}
			attIndex++;
		...
		for (int j = 0; j < instances.numClasses(); j++)
			m_Priors[j] = (m_Priors[j] + 1) / (sum + (double) instances.numClasses());
	}
