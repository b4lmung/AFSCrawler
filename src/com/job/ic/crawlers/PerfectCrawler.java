...
public class PerfectCrawler {
	private static Logger logger = Logger.getLogger(PerfectCrawler.class);
...
//	public static UrlDAO ud;
	...
	public static synchronized void addPage(final double score, final String url) {
		...
		if (PerfectCrawler.rel + PerfectCrawler.non > PerfectCrawler.limitPage) {
			System.exit(0);
		}
	...
	public static void main(final String[] args) throws Exception {
		...
		System.out.println(String.valueOf(PerfectCrawler.rel) + "\t" + PerfectCrawler.non);
	...
	public static void softFocusedThread() {
		...
		while (queue.size() > 0) {
			...
			PerfectCrawler.logger.info(String.format("%s\t%.2f\t%s\tParent:\t%.2f", url, score, progress(), qObj.getScore()));
...
//						logger.error(">>>>>>>>" + score + "\t" + l.getLinkUrl());
		...
		PerfectCrawler.logger.info("finished");
