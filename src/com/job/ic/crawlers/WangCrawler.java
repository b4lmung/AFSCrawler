...
public class WangCrawler {
	private static Logger logger = Logger.getLogger(WangCrawler.class);
	...
	public static synchronized void addPage(final double score, final String url) {
		...
		if (WangCrawler.rel + WangCrawler.non > WangCrawler.limitPage) {
			System.exit(0);
		}
	...
	public static void main(final String[] args) throws Exception {
		...
		System.out.println(String.valueOf(WangCrawler.rel) + "\t" + WangCrawler.non);
	...
	public static void softFocusedThread() {
		...
		while (queue.size() > 0) {
			...
			WangCrawler.logger.info(String.format("%s\t%.2f\t%s\tParent:\t%.2f", url, score, progress(), qObj.getScore()));
...
//						if (score > 0.5)
		...
		WangCrawler.logger.info("finished");
